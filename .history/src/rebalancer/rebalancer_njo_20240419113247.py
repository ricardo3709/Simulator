...
def rebalancer_njo(reqs: List[Req], vehs: List[Veh], system_time: float):
    ...
    if len(rejected_reqs) == 0 or len(avaliable_vehs) == 0: #no rejected requests or no avaliable vehicles, no need to rebalance
        return
    ...
    for veh in avaliable_vehs: # clear the schedule of vehicles
        veh.schedule = []
...
def compute_candidate_veh_req_pairs(reqs: List[Req], vehs:List[Veh], system_time: float) \
    ...
    for req in reqs:
        for veh in vehs: 
            ...
            rebalancing_sche, cost = compute_schedule(veh, req)
            candidate_veh_req_pairs.append([veh, req, rebalancing_sche, cost, 0.0]) #vt_pair = [veh, trip, sche, cost, score]
...
def compute_schedule(veh: Veh, req: Req):
    ...
    rebalancing_sche = copy.deepcopy(veh.schedule) 
    assert rebalancing_sche != None #DEBUG CODE, current_schedule should be None
    # assert len(current_schedule) < 5 #DEBUG CODE

    rebalancing_sche.append([req.Ori_id, 1, req.Num_people, req.Latest_PU_Time, req.Shortest_TT])
    time_cost = retrive_TimeCost(veh.current_node, req.Ori_id)
    return rebalancing_sche, time_cost
