...
def rebalancer_ilp_assignment(self, veh_trip_pairs: List[Tuple[Veh, List[Req], List[Tuple[int, int, int, float]], float, float]], #list[[Veh, list[Req], list[(int, int, int, float)], float, float]]
    ...
    for i in range(len(veh_trip_pairs)): #veh_trip_pairs = [veh, trip, sche, cost, score], len(veh_trip_pairs) = len(veh_trip_pairs_check)
        ...
        object_score += veh_trip_pairs_check[i] * (time_to_origin) + (1.0 - veh_trip_pairs_check[i]) * self.config.get('REBALANCER_PENALTY')
