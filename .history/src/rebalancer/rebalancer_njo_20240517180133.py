...
def rebalancer_njo(self, reqs: List[Req], vehs: List[Veh], system_time: float):
...
def compute_candidate_veh_req_pairs(self, reqs: List[Req], vehs:List[Veh], system_time: float) \
    ...
    for req in reqs:
        available_veh = []
        for veh in vehs: 
            # Check if the vehicle can reach the origin node before the self.config.get('MAX_DELAY_REBALANCE')
            time_to_origin = self.route_func.get_timeCost(veh.current_node, req.Ori_id)
            if time_to_origin > self.config.get('MAX_DELAY_REBALANCE'):
                continue
            available_veh.append([veh, time_to_origin])

        if len(available_veh) == 0: #no vehicle can reach the origin node before the self.config.get('MAX_DELAY_REBALANCE')
            continue

        if self.config.get('HEURISTIC_ENABLE'): # enable heuristic method to accelerate
            # if too many vehicles, we can use a heuristic to reduce the number of vehicles to consider
            if len(available_veh) > self.config.get('MAX_NUM_VEHICLES_TO_CONSIDER'):
                available_veh.sort(key = lambda x: x[1])
                available_veh = available_veh[:self.config.get('MAX_NUM_VEHICLES_TO_CONSIDER')]
...
def compute_schedule(self, veh: Veh, req: Req):
    ...
    time_cost = self.route_func.get_timeCost(veh.current_node, req.Ori_id)
