...
def assign_orders_through_sba(self,current_cycle_requests: List[Req], vehs: List[Veh], system_time: float, num_of_rejected_req_for_nodes_dict_movingAvg: dict, num_of_generate_req_for_nodes_dict_movingAvg: dict):
...
def compute_candidate_veh_req_pairs(self,current_cycle_requests: List[Req], vehs:List[Veh], system_time: float) \
    ...
    for req in current_cycle_requests:
        ...
        if self.config.get('HEURISTIC_ENABLE'): # enable heuristic method to accerlate =
...
def prune_candidate_veh_req_pairs(self,candidate_veh_req_pairs):
    if self.config.get('DEBUG_PRINT'):
...
def immediate_reject_unassigned_requests(self,current_cycle_requests: List[Req], assigned_reqs: List[Req],  num_of_rejected_req_for_nodes_dict_movingAvg):
    if self.config.get('DEBUG_PRINT'):
        print("                *Immediate rejecting unassigned orders...", end=" ")

    rej_reqs_per_nodes_dict = {i: 0 for i in range(1, self.config.get('NUM_NODES_MANHATTAN')+1)} # {node_id: num_rej_req} used in reward calculation
    for req in current_cycle_requests:
