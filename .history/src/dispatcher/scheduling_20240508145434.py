...
def test_constraints(schedule: list, veh: Veh): 
    ...
    for node in schedule:
        ...
        time_to_next_node = get_timeCost(current_node, node[0]) #time to travel to node
        accumulated_time += time_to_next_node #time to travel to node + current time
        time_marks.append([node[5], accumulated_time]) #[req_ID, accumulated_time when visit this node]
        ...
        if node[1] == 1: #PU
            ...
            #2.2 Max Waiting for Dropoff BUG
            PU_time_mark = [mark for mark in time_marks if mark[0] == node[5]][0][1] #find the PU time mark for this DO node
            travel_time_for_this_pair = accumulated_time - PU_time_mark #time to travel from PU to DO
            if travel_time_for_this_pair > MAX_DETOUR_TIME + node[4] : #if the time to travel to node exceeds the max detour+shortest time
                return False
