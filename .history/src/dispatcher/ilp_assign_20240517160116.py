...
def reward_function(new_schedule: list, veh: Veh, num_of_rejected_req_for_nodes_dict_movingAvg, num_of_generate_req_for_nodes_dict_movingAvg):
    reward = 0.0
    #using Last Node for reward calculation
    last_node = new_schedule[-1]
    # generation_rate = get_rejection_generation_rate_smooth(last_node[0], REJ_LAYERS, num_of_generate_req_for_nodes_dict_movingAvg)
    rejection_rate = get_rejection_generation_rate_smooth(last_node[0], REJ_LAYERS, num_of_rejected_req_for_nodes_dict_movingAvg)
...
def get_rejection_generation_rate_smooth(target_node: int, num_layers: int, rate_dict: dict):
    ...
    for node in considered_nodes:
        smooth_rejection_rate += np.sum(rate_dict[node])/(PSI + get_timeCost(target_node, node))
