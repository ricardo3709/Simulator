...
def reward_function(new_schedule: list, veh: Veh, num_of_rejected_req_for_nodes_dict_movingAvg, num_of_generate_req_for_nodes_dict_movingAvg):
    ...
    if REWARD_TYPE == 'REJ':
        reward = get_rejection_generation_rate_smooth(last_node[0], REJ_LAYERS, num_of_rejected_req_for_nodes_dict_movingAvg)
    else: 
        reward = get_rejection_generation_rate_smooth(last_node[0], REJ_LAYERS, num_of_generate_req_for_nodes_dict_movingAvg)
