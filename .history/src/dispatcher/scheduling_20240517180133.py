...
def compute_schedule(self, veh: Veh, req: Req): 
    ...
    if self.config.get('HEURISTIC_ENABLE') and len(current_schedule) > self.config.get('MAX_SCHEDULE_LENGTH'): #if the vehicle has more than 30 nodes in the schedule
...
def compute_schedule_normal(self, veh: Veh, req: Req, current_schedule: list):
    # feasible_schedules = []
    best_schedule = None
...
def compute_schedule_by_half(self, veh: Veh, req: Req, current_schedule: list):
    feasible_schedules = []
    best_schedule = None
...
def score_vt_pair_with_delay(self, candidate_veh_trip_pairs: list):
...
def compute_schedule_delay(self, schedule:list, veh:Veh): #sum(time of nodes to nodes) - sum(shortest time for all trips)
...
def compute_schedule_time_cost(self, schedule: list): 
    total_schedule_time = 0.0
    for i in range(len(schedule) - 1):
        total_schedule_time += self.route_func.get_timeCost(schedule[i][0], schedule[i+1][0])

    return total_schedule_time

def compute_extra_delay(self, inserted_schedule: list, insert_position: int, veh: Veh):
    if insert_position == len(inserted_schedule) - 1: #if the inserted node is the last node
        extra_delay = self.route_func.get_timeCost(inserted_schedule[insert_position-1][0], inserted_schedule[insert_position][0]) # use time to insert node as extra delay
        return extra_delay
    elif insert_position == 0: #if the inserted node is the first node
        new_time = self.route_func.get_timeCost(veh.current_node, inserted_schedule[0][0]) + self.route_func.get_timeCost(inserted_schedule[0][0], inserted_schedule[1][0])
        original_time = self.route_func.get_timeCost(veh.current_node, inserted_schedule[1][0]) 
        extra_delay = new_time - original_time
        return extra_delay
    else:
        original_time = self.route_func.get_timeCost(inserted_schedule[insert_position-1][0], inserted_schedule[insert_position+1][0])
        new_time = self.route_func.get_timeCost(inserted_schedule[insert_position-1][0], inserted_schedule[insert_position][0]) + self.route_func.get_timeCost(inserted_schedule[insert_position][0], inserted_schedule[insert_position+1][0])
        extra_delay = new_time - original_time
        return extra_delay

def test_constraints(self, schedule: list, veh: Veh): 
    ...
    #     time_to_next_node = self.route_func.get_timeCost(current_node, node[0]) #time to travel to node
...
def insert_request_into_schedule_half(self, schedule: list, request: Req, PU_node_position: int, DO_node_position):
...
def insert_request_into_schedule(self, schedule: list, request: Req, PU_node_position: int, DO_node_position: int):
        PU_node = [request.Ori_id, 1, request.Num_people, request.Latest_PU_Time, request.Shortest_TT, request.Req_ID, 'NEW_PU']
        DO_node = [request.Des_id, -1, request.Num_people, request.Latest_DO_Time, request.Shortest_TT, request.Req_ID, 'NEW_DO']
...
def upd_schedule_for_vehicles_in_selected_vt_pairs(self, candidate_veh_trip_pairs: list,
