...
def ilp_assignment(veh_trip_pairs, reqs, vehs, num_of_rejected_req_for_areas_dict_movingAvg, num_of_generate_req_for_areas_dict_movingAvg, config):
    ...
    for i, req in enumerate(reqs):
        model.addConstr(gp.quicksum(veh_trip_pairs_check[j] for j, vtp in enumerate(veh_trip_pairs) if req in vtp[1]) <= 1)

    # Objective: Minimize the total cost, considering penalties for unassigned vehicle-trip pairs.
    object_score = gp.quicksum(veh_trip_pairs_check[i] * (vtp[4] - REWARD_THETA * reward_function(vtp, num_of_rejected_req_for_areas_dict_movingAvg, num_of_generate_req_for_areas_dict_movingAvg, config))
...
def reward_function(new_schedule: list, veh: Veh, num_of_rejected_req_for_areas_dict_movingAvg, num_of_generate_req_for_areas_dict_movingAvg, config: ConfigManager):
    ...
    # else: 
    #     reward = np.sum(num_of_generate_req_for_areas_dict_movingAvg[area_id])

    # Using Smooth Rejection Rate of Last Node as Reward
    # if REWARD_TYPE == 'REJ':
    #     reward = get_rejection_generation_rate_smooth(last_node[0], NODE_LAYERS, num_of_rejected_req_for_areas_dict_movingAvg)
    # else: 
