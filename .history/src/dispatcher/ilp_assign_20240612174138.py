...
def anticipatory_cost(new_schedule: list, veh: Veh):
    ...
    length_diff = compute_schedule_time_cost(new_schedule, PU_position, DO_position, veh)
...
def compute_schedule_diff(new_schedule, PU_position, DO_position, veh):
    ...
    old_schedule = [new_schedule[idx] for idx in range(len(new_schedule)) if idx != PU_position or idx != DO_position]
