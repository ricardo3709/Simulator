...
from src.simulator.route_functions import RouteFunctions
...
def __init__(self):
    ...
    self.route_func = RouteFunctions()

def ilp_assignment(self, veh_trip_pairs: List[Tuple[Veh, List[Req], List[Tuple[int, int, int, float]], float, float]], #list[[Veh, list[Req], list[(int, int, int, float)], float, float]]
                #    considered_rids: List[int],
                   reqs: List[Req],
                   vehs: List[Veh],
                   num_of_rejected_req_for_nodes_dict_movingAvg,
                   num_of_generate_req_for_nodes_dict_movingAvg) -> List[int]:
    ...
    for i in range(len(veh_trip_pairs)): #veh_trip_pairs = [veh, trip, sche, cost, score], len(veh_trip_pairs) = len(veh_trip_pairs_check)
        ...
        # time_to_origin = self.route_func.get_timeCost(veh_trip_pairs[i][0].current_node, veh_trip_pairs[i][1].Ori_id) #time to travel to origin node, also need to be minimized
        # # object_score = Delay + Time_to_Origin(Pickup) + Penalty_of_Ignoring, for each veh_trip_pair
        # object_score += veh_trip_pairs_check[i] * (veh_trip_pairs[i][4] + time_to_origin) + (1.0 - veh_trip_pairs_check[i]) * self.config.get('PENALTY')
        
        # Anticipatory Method Objective 
        
        cost = anticipatory_cost(veh_trip_pairs[i][2], veh_trip_pairs[i][0]) 
        reward = reward_function(veh_trip_pairs[i][2], veh_trip_pairs[i][0], num_of_rejected_req_for_nodes_dict_movingAvg, num_of_generate_req_for_nodes_dict_movingAvg)
        anticipate_method_object_score = cost - self.config.get('REWARD_THETA') * reward
        object_score += veh_trip_pairs_check[i] * anticipate_method_object_score + (1.0 - veh_trip_pairs_check[i]) * self.config.get('PENALTY')
...
def anticipatory_cost(self, new_schedule: list, veh: Veh):
    # new req
    PU_position = None
    DO_position = None
    ...
    first_term = self.config.get('PW') * t_wait_new + self.config.get('PV') * detour_new

    # For insertion in empty schedule
    if len(new_schedule) == 2: # only newly inserted req
        anticipatory_cost_value = first_term + self.config.get('PO') * self.route_func.get_timeCost(new_schedule[0][0], new_schedule[1][0])
    ...
    second_term = self.config.get('PW') * extra_delay + self.config.get('PV') * extra_detour

    # Third term in objective function
    third_term = self.config.get('PO') * (extra_delay_NEWDO + extra_delay_NEWPU)
    
    # Anticipatory Cost Value
    anticipatory_cost_value = first_term + second_term + third_term
    return anticipatory_cost_value

def get_delay_and_detour_for_new_req(self, new_schedule: list, veh: Veh, PU_position: int, DO_position: int):
    time_to_first_node = self.route_func.get_timeCost(veh.current_node, new_schedule[0][0])
...
def get_req_pair_dict(self, schedule: list):
...
def get_extra_delay_and_detour(self, schedule: list, veh: Veh, req_pair_dict: dict, PU_position: int, DO_position: int, extra_delay_NEWPU: float, extra_delay_NEWDO: float):
    extra_delay= 0.0
    extra_detour = 0.0
...
def reward_function(self, new_schedule: list, veh: Veh, num_of_rejected_req_for_nodes_dict_movingAvg, num_of_generate_req_for_nodes_dict_movingAvg):
    reward = 0.0
    ...
    if self.config.get('REWARD_TYPE') == 'REJ':
        reward = get_rejection_generation_rate_smooth(last_node[0], self.config.get('NODE_LAYERS'), num_of_rejected_req_for_nodes_dict_movingAvg)
    else: 
        reward = get_rejection_generation_rate_smooth(last_node[0], self.config.get('NODE_LAYERS'), num_of_generate_req_for_nodes_dict_movingAvg)
    

    return reward

def get_rejection_generation_rate_smooth(self, target_node: int, num_layers: int, rate_dict: dict):
    considered_nodes = self.route_func.get_surrounding_nodes(target_node, num_layers)
    smooth_rate = 0.0
    for node in considered_nodes:
        smooth_rate += np.sum(rate_dict[node])/(self.config.get('PSI') + self.route_func.get_timeCost(target_node, node))

    return smooth_rate


def greedy_assignment(self, veh_trip_pairs: List[Tuple[Veh, List[Req], List[Tuple[int, int, int, float]], float, float]]) -> List[int]:
    if self.config.get('DEBUG_PRINT'):
