...
class Simulator_Platform(object):
    ...
    def __init__(self, simulation_start_time_stamp: float, config: ConfigManager):
        ...
        self.end_time = self.start_time + self.config.get('SIMULATION_DURATION') 
        self.recorded_end_time = self.start_time
        self.total_time_step = self.config.get('SIMULATION_DURATION')  // self.config.get('TIME_STEP')
        self.accumulated_request = []
        self.rejected_reqs = []
        # Use deque to achieve moving average. 12 means 3 minutes
        self.num_of_generate_req_for_nodes_dict_movingAvg = {i: deque(maxlen=self.config.get('MOVING_AVG_WINDOW')) for i in range(1, self.config.get('NUM_NODES_MANHATTAN')+1)} # {node_id: generate_req} used in reward calculation
        self.num_of_rejected_req_for_nodes_dict_movingAvg = {i: deque(maxlen=self.config.get('MOVING_AVG_WINDOW')) for i in range(1, self.config.get('NUM_NODES_MANHATTAN')+1)} # {node_id: rejected_req} used in reward calculation
        ...
        if self.config.get('MAP_NAME') == 'Manhattan': # [ReqID Oid Did ReqTime Size]
            print(f"[INFO] Loading ManhattanData...")
            reqs = pd.read_csv(self.config.get('PATH_MANHATTAN_REQUESTS'))
            ...
            node_id_uniform = list(range(1,self.config.get('NUM_NODES_MANHATTAN')+1, self.config.get('NUM_NODES_MANHATTAN') // self.config.get('FLEET_SIZE[0]')))
            for i in range(self.config.get('FLEET_SIZE[0]')):
                # node_id = np.random.randint(1, self.config.get('NUM_NODES_MANHATTAN')+1)
                self.vehs.append(Veh(i, self.system_time, node_id_uniform[i], self.config.get('VEH_CAPACITY[0]')))
            print(f"[INFO] Vehicles Initialization finished")

        elif self.config.get('MAP_NAME') == 'SmallGrid':
            print(f"[INFO] Loading SmallGridData...")
            reqs = pd.read_csv(self.config.get('PATH_SMALLGRID_REQUESTS'))
            reqs_data = reqs.to_numpy()
            for i in range(reqs_data.shape[0]):
            ...
            for i in range(self.config.get('FLEET_SIZE[0]')):
                ...
                self.vehs.append(Veh(i, self.system_time, node_id, self.config.get('VEH_CAPACITY[0]')))
            print(f"[INFO] Vehicles Initialization finished")
        ...
        if self.config.get('DISPATCHER') == "SBA":
            self.dispatcher = DispatcherMethod.SBA
        else:
            assert (False and "[ERROR] WRONG self.config.get('DISPATCHER') SETTING! Please check the name of dispatcher in config!")
        if self.config.get('REBALANCER') == "NONE":
            self.rebalancer = RebalancerMethod.NONE
        elif self.config.get('REBALANCER') == "NPO":
            self.rebalancer = RebalancerMethod.NPO
        elif self.config.get('REBALANCER') == "NJO":
            self.rebalancer = RebalancerMethod.NJO
        else:
            assert (False and "[ERROR] WRONG self.config.get('REBALANCER') SETTING! Please check the name of rebalancer in config!")

        if self.config.get('HEURISTIC_ENABLE'):
            print(f"[INFO] Heuristic method is enabled.")
        print(f"[INFO] Platform is ready. ")


    def run_simulation(self):
        print(f"current theta: {self.config.get('REWARD_THETA')}")      
        simulation_end_step = int(self.total_time_step)+1  
        cool_down_step = int(self.config.get('COOL_DOWN_DURATION') // self.config.get('TIME_STEP'))
        cool_down_flag = True
        for current_time_step in tqdm(range(max(int(self.start_time//self.config.get('TIME_STEP')),1), simulation_end_step + cool_down_step, 1), desc=f"Ricardo's Simulator"):
            self.system_time = current_time_step * self.config.get('TIME_STEP')
    ...
    def run_cycle(self, cool_down_flag):
        ...
        if self.dispatcher == DispatcherMethod.SBA:
            assign_orders_through_sba(self.accumulated_request, self.vehs, self.system_time, self.num_of_rejected_req_for_nodes_dict_movingAvg, self.num_of_generate_req_for_nodes_dict_movingAvg)
    ...
    def prune_requests(self):
        if self.config.get('DEBUG_PRINT'):
    ...
    def get_current_cycle_request(self, current_time ) -> list:
        ...
        gen_reqs_per_nodes_dict = {i: 0 for i in range(1, self.config.get('NUM_NODES_MANHATTAN')+1)} # {node_id: num_gen_req} used in reward calculation
        for req in self.reqs:
            #EXP: to make it faster. Assume the requests are sorted by Req_time    
            if req.Req_time > current_time:
                break

            elif current_time - self.config.get('TIME_STEP') <= req.Req_time:
        ...
        for node_id in range(1,self.config.get('NUM_NODES_MANHATTAN')+1):
    ...
    def create_report(self):
        ...
        print(f"Reward Theta: {self.config.get('REWARD_THETA')}")
    ...
    def create_video(self):
        ...
        FILE_TYPE = '.mp4'
        # BASEDIR = osp.dirname(osp.abspath(__file__)) + '/'
        OUTPUT_NAME = str(self.config.get('FLEET_SIZE[0]')) + 'Vehs_' + str(self.config.get('VEH_CAPACITY[0]')) + 'Ppl_' + str(self.config.get('MAX_PICKUP_WAIT_TIME')) + 'MaxWait_' + str(self.config.get('MAX_DETOUR_TIME')) + 'MaxDetour_' + self.config.get('REBALANCER') + 'Rebalancer_' + self.config.get('DISPATCHER') + 'Dispatcher' + FILE_TYPE
