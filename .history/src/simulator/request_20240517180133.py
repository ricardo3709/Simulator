...
class Req(object):
    ...
    #     self.Req_ID = Req_ID
    #     self.Shortest_TT = self.route_func.get_timeCost(self.Ori_id, self.Des_id)
    ...
    def __init__(self, Req_ID: int, Ori_id: int, Des_id: int, Req_time: int, Num_people: int):
        ...
        self.Req_ID = Req_ID
        self.Shortest_TT = self.route_func.get_timeCost(self.Ori_id, self.Des_id)
        self.Latest_PU_Time = self.Req_time + self.config.get('MAX_PICKUP_WAIT_TIME')
        self.Latest_DO_Time = self.Req_time + self.config.get('MAX_PICKUP_WAIT_TIME') + self.Shortest_TT + self.config.get('MAX_DETOUR_TIME') 
        self.Status = OrderStatus.PENDING
