...
def get_route(origin: int, destination: int):
        if MAP_NAME == "SmallGrid":
            ...
            route, _ = copy.deepcopy(get_route_by_matrix(origin, destination, ALL_PATH_TABLE, ALL_PATH_TIME_MATRIX))
        else:
...
def get_route_by_matrix(Oid: int, Did: int, all_path_matrix, all_path_time_matrix):
    current_node = Oid
    route = [Oid]
    # time_cost = 0.0
