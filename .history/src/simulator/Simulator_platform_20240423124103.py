...
class Simulator_Platform(object):
    ...
    def run_simulation(self):      
        ...
        for current_time_step in tqdm(range(max(int(self.start_time),1), simulation_end_step + cool_down_step, 1), desc=f"Ricardo's Simulator"):
            ...
            if cool_down_flag and current_time_step > (simulation_end_step - cool_down_step):
                print("Cooling down...")
                cool_down_flag = False
            self.run_cycle()
            self.statistic.veh_positions.append(self.get_all_veh_positions())
    ...
    def get_all_veh_positions(self) -> list:
        veh_positions = []
        for veh in self.vehs:
