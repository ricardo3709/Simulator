...
class Veh(object):
    ...
    Attributes:
        ...
        time_steps: time steps from the beginning of the simulation
        ...
        ---- Try to use iteration time steps instead ----
        state_time: system time at current state  (Replaced by veh_time)
        ---- Longtitute&Latitude ----
        lat: current lngitude
        lng: current longtitude
        ---- Below only for statistics ----
    ...
    def __init__(self, id: int, nid: int, lng: float, lat: float, capacity: int, current_system_time: int): #Replace system time with current_system_time
        self.id = id
    ...
    def move_to_time(self, current_system_time: int, update_vehicle_statistics: bool) -> list:
        dT = current_system_time - self.veh_time
        if dT <= 0: #check whether veh_time is normal. If not, return an empty list.
            return []
        ...
        while dT > 0 and len(self.route) > 0:
            ...
            if leg.t < dT: #TimeCost of current leg is less than the time left
                dT -= leg.t #Time left after finishing the current leg
                self.veh_time += leg.t
                ...
                self.update_service_status_after_veh_finish_a_leg(finished_legs, leg)
                ...
                while dT > 0 and len(leg.steps) > 0:
                    ...
                    if step.t < dT:
                        ...
                        if len(leg.steps) == 0:
                            ...
                            self.update_service_status_after_veh_finish_a_leg(finished_legs, leg)
                            print("corner case: leg.t extremely small, but still larger than dT")
                            break

                    # the vehicle has to stop somewhere within the step
                    # the step is not finished yet
                    else:
                        pct = dT / step.t
                        ...
                        # WIP: how to find exact location without using longitute and latitude?
                        self.cut_step(pct)
                        self.jump_to_location(step.nid_pair[0], step.geo_pair[0][0], step.geo_pair[0][1])
                        self.veh_time = current_system_time
                        return finished_legs

        # We've finished the whole schedule.
        assert dT > 0 or np.isclose(dT, 0.0)
        assert self.veh_time < current_system_time or np.isclose(self.veh_time, current_system_time)
    ...
    def update_service_status_after_veh_finish_a_leg(self, finished_legs: list[(int, int, int)], leg: Leg):
        self.jump_to_location(leg.tnid)
        self.load += leg.pod
        if leg.rid != -2:
            finished_legs.append((leg.rid, leg.pod, self.veh_time))
            ...
            if leg.pod == 1:
                ...
                # if DEBUG_PRINT:
                #     print(f"            +vehicle #{self.id} picked up req #{leg.rid} at {self.veh_time}s")
            elif leg.pod == -1:
                self.new_dropped_rids.append(leg.rid)
                self.onboard_rids.remove(leg.rid)
                # if DEBUG_PRINT:
                #     print(f"            +vehicle #{self.id} dropped up req #{leg.rid} at {self.veh_time}s")
    ...
    def build_route(self, sche: list[(int, int, int, float)]):
        ...
        # WIP: Not sure what are these about:
        ...
        if self.step_to_nid:
            ...
            ddl = self.veh_time + self.step_to_nid.t
