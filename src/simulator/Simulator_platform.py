...
from src.simulator.data_processing import *
...
class Simulator_Platform(object):
    ...
    def __init__(self, simulation_start_time_stamp: float, config: ConfigManager):
        ...
        if MAP_NAME == 'Manhattan': # [ReqID Oid Did ReqTime Size]
            ...
            # reqs = pd.read_csv(PATH_COMBINED_REQUESTS)
    ...
    def run_simulation(self):
        ...
        for current_time_step in tqdm(range(int(self.start_time//TIME_STEP), simulation_end_step, 1), desc=f"Ricardo's Simulator"):
            ...
            if current_time_step % 10 == 0: #record rej_rate every 10 steps(2.5 mins)
                ...
                action = self.get_action(self.action_seed) #random_policy
                reward = self.get_reward() #past_rej[-2] - past_rej[-1]

                current_theta = self.config.get("REWARD_THETA") 
                new_theta = current_theta + action
                self.config.set("REWARD_THETA", new_theta)
                print(f"current theta: {current_theta}, action: {action}, new theta: {new_theta}, reward: {reward}")

                self.action_seed = np.random.randint(0,6) #generate random action seed
    ...
    def run_cycle(self):
        ...
        if self.dispatcher == DispatcherMethod.SBA:
            assign_orders_through_sba(self.accumulated_request, self.vehs, self.system_time, self.num_of_rejected_req_for_areas_dict_movingAvg, self.num_of_generate_req_for_areas_dict_movingAvg, self.config, self.statistic)
    ...
    def create_report(self,runtime):
        ...
        print(f"total reqs: {self.statistic.total_rejected_requests + self.statistic.total_picked_requests}")
        print(f"avg cost:{self.statistic.total_cost/self.statistic.total_picked_requests}")
    ...
    def write_theta_rej_results_to_file(self, results):
        ...
        file_name = f"penalty_{PENALTY}_window_{MOVING_AVG_WINDOW}_{duration}hours_scale1_10init.txt"
    ...
    def write_accumulated_rej_rate_to_file(self, theta, rej_rate_by_5mins_accumulated):
        ...
        REWARD_THETA = self.config.get("REWARD_THETA")
        duration = int(SIMULATION_DURATION/3600)
        directory = os.path.join(os.getcwd(), "results/toggle_theta")
        # 确保结果目录存在
        os.makedirs(directory, exist_ok=True)

        # file_name = f"rej_rate_by_5mins_accumulated_theta_{theta}.txt"
        file_name = f"detailed_theta_{REWARD_THETA}_{TOGGLE_THETA_VALUE}_penalty_{PENALTY}_window_{MOVING_AVG_WINDOW}_{duration}hours.txt"
        path = os.path.join(directory, file_name)
    ...
    def get_reward(self):
        past_rej = self.current_cycle_rej_rate
        if len(past_rej) < 2:
            return 0.0
        reward = past_rej[-2] - past_rej[-1]
        return reward
    
    def get_action(self,action_seed):
        # action_seed: int
        # return: float
        policy = {0: 0.01, 1: 0.05, 2: 0.1, 3: -0.01, 4: -0.05, 5: -0.1}
        return policy[action_seed]
    
    def data_loger(self, next_state, reward, action, prev_rej):
        reward_col = np.full((63, 1), reward)
        action_col = np.full((63, 1), action)
        prev_rej_col = np.full((63, 1), prev_rej)
